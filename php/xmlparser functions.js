window.phpIntellisense.xmlParserFunctions = [
    window.cmSnip(`utf8_decode($\{string});$\{}`, {
        label: "utf8_decode()",
        detail: "XML Parser",
        type: "method",
...
This function decodes a string, previously encoded with the <code>utf8_encode()</code> function, back to ISO-8859-1.`
    ...
    window.cmSnip(`utf8_encode($\{string});$\{}`, {
        label: "utf8_encode()",
        detail: "XML Parser",
        type: "method",
...
However, it is not always possible to transfer a Unicode character to another computer reliably. UTF-8 has been developed to transfer a Unicode character from one computer to another.`
    ...
    window.cmSnip(`xml_get_current_column_number($\{resource});$\{}`, {
    ...
    window.cmSnip(`xml_get_current_line_number($\{resource});$\{}`, {
    ...
    window.cmSnip(`xml_set_default_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_element_handler($\{resource}, $\{callable}, $\{callable});$\{}`, {
        label: "xml_set_element_handler()",
        detail: "XML Parser",
        type: "method",
        info: `The xml_set_element_handler() function sets up the element handler for the XML parser.`
    }),
    window.cmSnip(`xml_set_end_namespace_decl_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_external_entity_ref_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_notation_decl_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_object($\{resource}, $\{object});$\{}`, {
        label: "xml_set_object()",
        detail: "XML Parser",
        type: "method",
        info: `The xml_set_object() function allows to use XML parser within an object.`
    }),
    window.cmSnip(`xml_set_processing_instruction_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_start_namespace_decl_handler($\{resource}, $\{callable});$\{}`, {
    ...
    window.cmSnip(`xml_set_unparsed_entity_decl_handler($\{resource}, $\{callable});$\{}`, {
